...
static int is_beegfs_path(const char *path) {
  int fd;
  ...
  if (!parent_dir) {
    log_debug(
        "[file_open_interceptor] Failed to allocate memory for parent_dir "
        "path\n");
    return 0;
  }
  ...
  if (last_slash)
    *last_slash = '\0';
    ...
    log_debug(
        "[file_open_interceptor] Invalid path: %s (no parent directory)\n",
        path);
    free(parent_dir);
    return 0;
  }
  ...
  if (fd < 0) {
    log_debug(
        "[file_open_interceptor] Failed to open parent directory: %s (errno: "
        "%d)\n",
        parent_dir, errno);
    free(parent_dir);
    return 0;
  ...
  if (is_beegfs) {
    ...
    if (beegfs_getConfigFile(fd, &config_file)) {
      log_debug(
          "[file_open_interceptor] BeeGFS detected for path: %s (config: %s)\n",
          path, config_file);
      free(config_file);
    } else {
      log_debug(
          "[file_open_interceptor] BeeGFS detected for path: %s (config "
          "unavailable)\n",
          path);
    }
  } else {
...
static int create_striped_file(const char *path, mode_t mode,
  ...
  *last_slash = '\0';
  ...
  log_debug(
      "[file_open_interceptor] Attempting to create striped file: %s "
  ...
  if (beegfs_createFile(fd, filename, mode, stripe_count, stripe_size)) {
    ...
    if (beegfs_getStripeInfo(fd, &pattern_type, &chunk_size, &num_targets)) {
      log_debug(
          "[file_open_interceptor] Successfully created striped file: %s\n"
          "  Pattern Type: %u\n"
          "  Chunk Size: %u\n"
          "  Number of Targets: %u\n",
      ...
      for (uint16_t i = 0; i < num_targets; i++) {
        ...
        if (beegfs_getStripeTarget(fd, i, &target_id, &node_id, &node_str)) {
          log_debug("  Target %u: ID=%u, NodeID=%u, Node=%s\n", i, target_id,
                    node_id, node_str ? node_str : "unknown");
          if (node_str) free(node_str);
        }
      }
    } else {
      log_debug(
          "[file_open_interceptor] Created file but failed to get stripe info: "
          "%s\n",
          path);
    }
  } else {
    log_debug(
        "[file_open_interceptor] Failed to create striped file: %s (errno: "
        "%d)\n",
        path, errno);
  }
...
static void log_file_operation(const char *path, int flags, mode_t mode,
                               int created, int striped) {
  char flag_desc[256] = "";
  ...
  if ((flags & O_RDWR) == O_RDWR)
    strcat(flag_desc, "O_RDWR ");
  else if (flags & O_RDONLY)
    strcat(flag_desc, "O_RDONLY ");
  else if (flags & O_WRONLY)
    strcat(flag_desc, "O_WRONLY ");

  if (created) {
    log_debug(
        "[file_open_interceptor] Created new file\n"
        "  Path: %s\n"
        "  Flags: %s(0x%x)\n"
        "  Mode: %o\n"
        "  BeeGFS Striping: %s\n",
        path, flag_desc, flags, mode, striped ? "Yes" : "No");
  } else if (flags & O_CREAT) {
    log_debug(
        "[file_open_interceptor] Opened existing file\n"
        "  Path: %s\n"
        "  Flags: %s(0x%x)\n"
        "  Mode: %o\n",
...
static int is_open_syscall(long syscall_number) {
  return syscall_number == SYS_open || syscall_number == SYS_openat ||
...
static int hook(long syscall_number, long arg0, long arg1, long arg2, long arg3,
  ...
  if (!is_open_syscall(syscall_number)) return 1; /* Ignore other syscalls */
  ...
  if ((flags & O_CREAT) && !(flags & O_EXCL) && is_beegfs_path(path)) {
    ...
    if (create_striped_file(path, mode, DEFAULT_STRIPE_COUNT,
...
static __attribute__((constructor)) void start(void) {
  ...
  if (log_path == NULL) log_path = "/tmp/file_open.log";

  log_fd = (int)syscall_no_intercept(
      SYS_open, log_path, O_CREAT | O_WRONLY | O_APPEND, (mode_t)0644);

  if (log_fd < 0) syscall_no_intercept(SYS_exit_group, 1);
